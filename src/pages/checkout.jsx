...
import { FaCheckCircle, FaExclamationTriangle, FaPlus, FaArrowRight } from 'react-icons/fa';
...
const Checkout = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                setError('');
                // Fetch cart items
                const cartDoc = await firebase.firestore().collection('carts').doc(user.uid).get();
                if (cartDoc.exists) {
    ...
    const handleSaveAddress = async (e) => {
        ...
        try {
            const docRef = await firebase.firestore().collection('addresses').add({
                ...newAddress,
                buyerId: user.uid,
                createdAt: firebase.firestore.FieldValue.serverTimestamp()
            });
